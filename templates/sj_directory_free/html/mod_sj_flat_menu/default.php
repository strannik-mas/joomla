	...
	for($i = 0; $i < count($menus); $i++){
		...
		$li = "<li ".$class.">";
 			$li .= "<div class='fm-item '>";
				...
				$icon_anchor_css = ($menus[$i]->menu_anchor != '')?'<i class="'.$menus[$i]->menu_anchor.'"></i>' : '';
				$divLink = "<div class='fm-link' ".$style."><a ".$target." href='".$menus[$i]->flink."'>".$icon_anchor_css.$icon_menu.$menus[$i]->title."</a></div>";
			$li.=$divLink;
			$li.= "</div>";
			echo $li;
				if($type_menu == 'flyout') {
