...
<div class="catItemView grid group<?php echo ucfirst($this->item->itemGroup); ?><?php echo ($this->item->featured) ? ' catItemIsFeatured' : ''; ?><?php if($this->item->params->get('pageclass_sfx')) echo ' '.$this->item->params->get('pageclass_sfx'); ?>">
    ...
    <?php if($this->item->params->get('catItemImage') ): ?>
	  ...
	  <div class="catItemImageBlock">
		    <a class="img"  href="<?php echo $this->item->link; ?>" title="<?php if(!empty($this->item->image_caption)) echo K2HelperUtilities::cleanHtml($this->item->image_caption); else echo K2HelperUtilities::cleanHtml($this->item->title); ?>">
		    	<?php 
	...
	<div class="main-item">
		...
		<span class="moduleItemDateCreated"><?php echo JHTML::_('date', $this->item->created , 'M d, Y'); ?></span>
		...
		<div class="rate-comment">
			...
			<div class="icomments">
				...
				<?php else: ?>
				<i class="fa fa-comment"></i>
				<a class="itemCommentsLink k2Anchor" href="<?php echo $this->item->link; ?>#itemCommentsAnchor">0</a>
				<?php endif; ?>
				<?php endif; ?>
			</div>
		</div>
		...
		<ul class="catToolbar">
			...
			<li>
				...
				<?php
	                $cat_title = $this->item->category->name;
	                $new_title = JString::strpos($cat_title, '|');
	                $newcat_title = ($new_title !== false) ? '<a href="' . $this->item->category->link . '" title="'.JString::substr($cat_title, 0, $new_title).'"' . JString::substr($cat_title, 0, $new_title) . '</a>': '<a href="' . $this->item->category->link . '" title="'.$this->item->category->name.'">' . $this->item->category->name . '</a>';
	                echo $newcat_title;
	            ?>
			</li>
			<?php } ?>
			...
			<li class="catItemHits">
				<?php
				if ($this->item->hits >1){?>
					<?php echo $this->item->hits; ?>&nbsp;<?php echo JText::_('K2_HITS'); ?>
				<?php } else { ?>
					<?php echo $this->item->hits; ?>&nbsp;<?php echo JText::_('K2_HIT'); ?>
				<?php } ?>					
			</li>
			<?php endif; ?>	
			...
			<li>
				...
				<?php else: ?>
					...
					<?php else: ?>
					<a class="itemCommentsLink k2Anchor" href="<?php echo $this->item->link; ?>#itemCommentsAnchor">						
						<?php echo JText::_('K2_NO_COMMENT'); ?></a>
					<?php endif; ?>
				<?php endif; ?>
			</li>
			<?php endif; ?>
		...
		<a class="read-more" href="<?php echo $this->item->link; ?>" title="<?php echo JText::_('READ_MORE_K2'); ?>"><?php echo JText::_('READ_MORE_K2'); ?></a>
		<?php endif; ?>
